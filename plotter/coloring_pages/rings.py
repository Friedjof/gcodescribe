...
from .geometry import arc, bud, circle, closed_polygon, diamond, heart, leaf, petal, polar, regular_polygon
...
def ring_petals(cx: float, cy: float, r0: float, r1: float, n: int, phase: float, rng: random.Random, detail: float) -> list[Polyline]:
    lines: list[Polyline] = []
    ...
    for k in range(n):
        ...
        if echo:
            lines.append(petal(cx, cy, mid, r0 + (r1 - r0) * 0.12, r0 + (r1 - r0) * 0.62, width * 0.55))
        if tip_dot:
...
def ring_pointed(cx: float, cy: float, r0: float, r1: float, n: int, phase: float, rng: random.Random, detail: float) -> list[Polyline]:
    lines: list[Polyline] = []
    ...
    for k in range(n):
        ...
        if vein:
            lines.append([polar(cx, cy, r0 + (r1 - r0) * 0.12, mid), polar(cx, cy, r0 + (r1 - r0) * 0.82, mid)])
    return lines


def ring_buds(cx: float, cy: float, r0: float, r1: float, n: int, phase: float, rng: random.Random, detail: float) -> list[Polyline]:
    lines: list[Polyline] = []
...
def ring_scallops(cx: float, cy: float, r0: float, r1: float, n: int, phase: float, rng: random.Random, detail: float) -> list[Polyline]:
    ...
    for k in range(n):
        ...
        if echo:
            lines.append(_scallop(cx, cy, r0, r0 + (r1 - r0) * 0.55, a0 + (a1 - a0) * 0.18, a1 - (a1 - a0) * 0.18, bulge))
    return lines


def _scallop(cx: float, cy: float, r0: float, r1: float, a0: float, a1: float, bulge: float) -> Polyline:
    pts: list[Point] = []
    for i in range(13):
        t = i / 12
...
def ring_dots(cx: float, cy: float, r0: float, r1: float, n: int, phase: float, rng: random.Random, detail: float) -> list[Polyline]:
    lines: list[Polyline] = []
    mid_r = (r0 + r1) / 2
...
def ring_diamonds(cx: float, cy: float, r0: float, r1: float, n: int, phase: float, rng: random.Random, detail: float) -> list[Polyline]:
    lines: list[Polyline] = []
    ...
    for k in range(n):
        ...
        if nested:
            lines.append(diamond(cx, cy, mid, r0 + (r1 - r0) * 0.25, r1 - (r1 - r0) * 0.25, width * 0.5))
    return lines


def ring_spikes(cx: float, cy: float, r0: float, r1: float, n: int, phase: float, rng: random.Random, detail: float) -> list[Polyline]:
    ...
    for k in range(n):
        ...
        lines.append(closed_polygon([polar(cx, cy, r0, base0), polar(cx, cy, r1, mid), polar(cx, cy, r0, base1)]))
        if inner_spike:
            lines.append(closed_polygon([
...
def ring_zigzag(cx: float, cy: float, r0: float, r1: float, n: int, phase: float, rng: random.Random, detail: float) -> list[Polyline]:
...
def ring_loops(cx: float, cy: float, r0: float, r1: float, n: int, phase: float, rng: random.Random, detail: float) -> list[Polyline]:
    lines: list[Polyline] = []
    mid_r = (r0 + r1) / 2
...
def ring_hearts(cx: float, cy: float, r0: float, r1: float, n: int, phase: float, rng: random.Random, detail: float) -> list[Polyline]:
    lines: list[Polyline] = []
    mid_r = (r0 + r1) / 2
    cell_w = math.tau * mid_r / n
...
def ring_cells(cx: float, cy: float, r0: float, r1: float, n: int, phase: float, rng: random.Random, detail: float) -> list[Polyline]:
...
def ring_waves(cx: float, cy: float, r0: float, r1: float, n: int, phase: float, rng: random.Random, detail: float) -> list[Polyline]:
    mid_r = (r0 + r1) / 2
...
def _wave_circle(cx: float, cy: float, mid_r: float, amp: float, n: int, phase: float) -> Polyline:
    samples = max(96, n * 14)
    pts = [polar(cx, cy, mid_r + amp * math.sin(n * (math.tau * i / samples) + phase), math.tau * i / samples) for i in range(samples)]
    return closed_polygon(pts)


def ring_fans(cx: float, cy: float, r0: float, r1: float, n: int, phase: float, rng: random.Random, detail: float) -> list[Polyline]:
...
def center_medallion(cx: float, cy: float, r: float, rng: random.Random, detail: float) -> list[Polyline]:
    ...
    if variant == "rosette":
        ...
        if detail > 0.4:
            for k in range(8):
                lines.append([polar(cx, cy, r * 0.62, k / 8 * math.tau), polar(cx, cy, r, k / 8 * math.tau)])
    elif variant == "star":
        ...
        pts = [polar(cx, cy, r * i / samples, direction * turns * math.tau * i / samples) for i in range(samples + 1)]
        lines.append(pts)
