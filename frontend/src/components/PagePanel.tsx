...
/**
 * The designer's page sidebar. With many pages a flat append-ordered list
 * becomes unmanageable, so this panel adds a name search, a sort selector
 * (custom / recently edited / name / created) and groups pages by their
 * calibration profile — the active profile first (open), foreign/archived
 * profiles in separate collapsible sections. In "custom order" the list can
 * be reordered by drag & drop. It keeps the active page scrolled into view.
 */
export default function PagePanel({
  ...
  const isCollapsed = (g: Group) =>
    collapsed[g.key] ?? (!g.active && !g.items.some((m) => m.id === activePageId));
  ...
  const renderItem = (m: PageMeta) => (
    <li
      ...
      onDragOver={
        canDrag && dragId && dragId !== m.id
          ? (e) => { e.preventDefault(); setDropId(m.id); }
          : undefined
      }
      onDrop={canDrag ? (e) => { e.preventDefault(); reorder(m.id); setDragId(null); setDropId(null); } : undefined}
  ...
  return (
    <aside className="paint-pages">
      <div className="paint-pages-head">
        <h3>{t("paint.pages")}</h3>
        <button className="ghost" title={t("paint.newPage")} onClick={onNew}>＋</button>
      </div>
      ...
      <div className="paint-pages-scroll">
        {groups.length === 0 ? (
          <p className="muted paint-pages-empty">{t("paint.noPagesFound")}</p>
        ) : (
          groups.map((g) =>
            multiGroup ? (
              <div className={"page-group" + (g.active ? " active-group" : "")} key={g.key}>
                <button
                  ...
                  onClick={() => setCollapsed((c) => ({ ...c, [g.key]: !isCollapsed(g) }))}
                  aria-expanded={!isCollapsed(g)}
                >
                  <span className="page-group-caret">{isCollapsed(g) ? "▸" : "▾"}</span>
                  <span className="page-group-name">{g.label}</span>
                  <span className="page-group-count">{g.items.length}</span>
                </button>
                {!isCollapsed(g) && <ul className="page-list">{g.items.map(renderItem)}</ul>}
              </div>
            ) : (
